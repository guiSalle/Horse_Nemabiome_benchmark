...
path <- "~/Documents/CYATHOMIX/WP3/NMB/MOCK_paper/"
setwd(path)
...
for(mxee in c(11,25)){
  for(truncL in c(200, 217)){
    for(BS in c(-1,16,32)){
      ...
      ### Read in seqtab
      seqtab_nochim = read.table(file = paste0('./data_for_sub/mock_DADA2/mock_dada2_R_',bc,'_mxee',mxee,'_trunc',truncL,'_BS',BS,'/output.tsv'),
                                 header=T)
  ...
  for( B in c(1:4)){
    for(k in c(10, 13, 15)){
      for(w in c(8, 9, 10)){                                                                                                                                    
        ...
        miniNofilt <- sort(list.files(paste0('./data_for_sub/mock_MINIMAP/mock_MINIMAPm_clus',id,'_B',B,'_k',k,'_w',w ), 
                                      pattern = "^COI.*L001_nofilt_genomecov_d.txt.gz", full.names = TRUE))
        miniFilt <- sort(list.files(paste0('./data_for_sub/mock_MINIMAP/mock_MINIMAPm_clus',id,'_B',B,'_k',k,'_w',w), 
...
miniNofilt <- sort(list.files(paste0('./data_for_sub/mock_MINIMAP/mock_MINIMAPm_clus',id,'_B',B,'_k',k,'_w',w ), 
                              pattern = "^COI.*L001_nofilt_genomecov_d.txt.gz", full.names = TRUE))
ministats <- sort(list.files(paste0('./data_for_sub/mock_MINIMAP/mock_MINIMAPm_clus',id,'_B',B,'_k',k,'_w',w ), 
...
miniNofilt <- sort(list.files('./data/life_stage/mock_MINIMAPm_clus99_B4_k10_w10/',
                              pattern = "^COI.*L001_nofilt_genomecov_d.txt.gz", full.names = TRUE))
ministats <- sort(list.files('./data/life_stage/mock_MINIMAPm_clus99_B4_k10_w10/',
                             pattern = "^COI.*idxstats.txt.gz", full.names = TRUE))
otumini = NULL
rcomini = NULL
...
seqtab_nochim = read.table(file = './data/life_stage/dada2_R_mxee11_trunc200_BS32/output.tsv', header=T)
dim(seqtab_nochim)
#[1] 345 100
