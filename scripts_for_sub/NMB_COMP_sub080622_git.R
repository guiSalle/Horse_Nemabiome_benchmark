...
for(mxee in c(11,25)){
  for(truncL in c(200, 217)){
    for(BS in c(-1,16,32)){
      ...
      ### Read in seqtab
      seqtab_nochim = read.table(file = paste0('./data/mock_DADA2/mock_dada2_R_',bc,'_mxee',mxee,'_trunc',truncL,'_BS',BS,'/output.tsv'),
                                 header=T)
  ...
  for( B in c(1:4)){
    for(k in c(10, 13, 15)){
      for(w in c(8, 9, 10)){                                                                                                                                    
        ...
        miniNofilt <- sort(list.files(paste0('./data/mock_MINIMAP/mock_MINIMAPm_clus',id,'_B',B,'_k',k,'_w',w ), 
                                      pattern = "^COI.*L001_nofilt_genomecov_d.txt.gz", full.names = TRUE))
        miniFilt <- sort(list.files(paste0('./data/mock_MINIMAP/mock_MINIMAPm_clus',id,'_B',B,'_k',k,'_w',w), 
...
miniNofilt <- sort(list.files(paste0('./data/mock_MINIMAP/mock_MINIMAPm_clus',id,'_B',B,'_k',k,'_w',w ), 
                              pattern = "^COI.*L001_nofilt_genomecov_d.txt.gz", full.names = TRUE))
ministats <- sort(list.files(paste0('./data/mock_MINIMAP/mock_MINIMAPm_clus',id,'_B',B,'_k',k,'_w',w ), 
...
psITS
# phyloseq-class experiment-level object
# otu_table()   OTU Table:         [ 12 taxa and 13 samples ]
